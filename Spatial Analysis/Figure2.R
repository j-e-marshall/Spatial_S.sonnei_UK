...
probbyspat<- pprob + theme_classic() + scale_x_continuous(name = "Pairwise Spatial Distance (km)")+labs(y="Probability of <2 Evolutionary Time", limits = c(0,NA),,expand = c(0, 0))+scale_y_continuous(limits = c(0,NA),expand = c(0, 0)) 
...
#plotting
ppop<-ggplot(aes(y=Chains, x=meanpopsize,colour = Group), data = totspatpop) +geom_point()+ geom_line()+ scale_color_manual(values = c("#b21819", "#088978"))
...
parea<-ggplot(aes(y=Chains, x=area,colour = Group), data = totspatpop)+geom_point() + geom_line()+ scale_color_manual(values = c("#b21819", "#088978"))
...
dat<- rbind(datanon,datapmsm )

pars<- matrix(nrow=3, ncol = 500)
for (i in 1:500){
  samp<- dat[,i]
  duse<-cbind(as.numeric(samp), dat[,501], as.numeric(dat[,503]))
  colnames(duse)<- c("value","Group", "popsize")
  duse<- data.frame(duse) %>% mutate(non = if_else(Group == "non-pMSM",0,1))
  linearreg<- lm(1/(as.numeric(value)) ~ log(as.numeric(popsize)) + duse$non, duse)
  pars[,i]<-linearreg$coefficients
  
}
dist_par2<- ecdf(pars[2,])
qt_0_par2 <- dist_par2(0)
...
quantile(pars[3,], probs = c(0.025, 0.975))
mean(pars[3,])

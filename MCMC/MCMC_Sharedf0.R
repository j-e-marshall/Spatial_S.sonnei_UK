...
## introduction
data.MCMC_AZ$yearF0 =array(rep(1, data.MCMC_AZ$nb_groups))
data.MCMC_AZ$yearIntroduction =array(rep(15, data.MCMC_AZ$nb_groups))
nb_geno = data.MCMC_AZ$nb_genotypes
...
for(i in 1:3) {
  ...
  fit_delay <- sampling(model.MCMC, data = data.MCMC_AZ, 
                        show_messages = TRUE, 
                        chains = 1,cores = 1,iter= 10000, chain_id = i,
                        control = list(adapt_delta = 0.97, max_treedepth = 10), init = list(list(f0 = f0_init( data.MCMC_AZ$nb_genotypes),fitness_genotypes_pre_switch = array(rep(rnorm(1,0,0.01),3)),fitness_genotypes_post_switch = array(rep(rnorm(1,0,0.01),3)))))
  fit = list(fit=fit_delay,
             data= data.MCMC_AZ)
  Chains=rstan::extract(fit$fit)
  saveRDS(Chains, file = paste0(name_file, '_chains_', i, '.rds'))
  saveRDS(data.MCMC_AZ, file = paste0(name_file, '_data_', i, '.rds'))
...
datindeplong$deltamean = datindeplong$Mean.x-datindeplong$Mean.y
datindeplong$deltauci = datindeplong$q972.5.x-datindeplong$q2.5.y
datindeplong$deltalci = datindeplong$q2.5.x-datindeplong$q972.5.y
colnames(datindeplong)[c(3,8)]<- c("Post Mean", "Pre Mean")
datindeplong_filt<- datindeplong %>% dplyr::select(Group, `Pre Mean`, q2.5.y, q972.5.y,`Post Mean`, q2.5.x, q972.5.x)
...
datindeplong_filt <- datindeplong_filt %>%
  mutate(
    estimate_CI_pre_exp = sprintf("%.2f (%.2f, %.2f)", exp(`Pre Mean`), exp(q2.5.y), exp(q972.5.y)),
    estimate_CI_post_exp = sprintf("%.2f (%.2f, %.2f)", exp(`Post Mean`), exp(q2.5.x), exp(q972.5.x))
  )

kable(
  datindeplong_filt %>% dplyr::select(Group, estimate_CI_pre_exp, estimate_CI_post_exp),
  digits = 2,
  col.names = c("Group", "Pre-2018 (95% CI)","Post-2018 (95% CI)")
)
kable(datindeplong_filt, digits = 2)
...
for(l in 1:17) {
  print(paste0('F0 = ', l))
  data.MCMC_AZ$yearF0 = array(rep(years[l], data.MCMC_AZ$nb_groups)) 
  for(y in 1:17) {
    if(l < y){
      print(paste0('First switch = ',l, y))
      data.MCMC_AZ$yearIntroduction = array(rep(years[y], data.MCMC_AZ$nb_groups)) 
      for(i in 1:3) {
        print(paste0('Running chain n = ', i))
        fit_delay <- sampling(model.MCMC, data = data.MCMC_AZ, 
                              show_messages = TRUE, 
                              chains = 1,cores = 1,iter= 10000, chain_id = i,
                              control = list(adapt_delta = 0.97, max_treedepth = 15), init = list(list(f0 = f0_init( data.MCMC_AZ$nb_genotypes),fitness_genotypes_pre_switch = array(rep(rnorm(1,0,0.01),3)),fitness_genotypes_post_switch = array(rep(rnorm(1,0,0.01),3)))))
        fit = list(fit=fit_delay,
                   data= data.MCMC_AZ)
        Chains=rstan::extract(fit$fit)
        saveRDS(Chains, file = paste0(name_file, '_chains_', i, '.rds'))
...
waicplotallazfilt<-ggplot(aes(x = reorder(switches,WAIC), y = WAIC), data = longwaic2) +geom_line(linetype= 2, color = "grey")+ geom_point(size=2)+theme_classic()+ theme(axis.text.x=element_text(angle=90, hjust=1))+geom_hline(aes(yintercept=c(420.4442+7)))+geom_hline(aes(yintercept=(422.0094+7)))+geom_hline(aes(yintercept=(423.5250+7)))
...
threshold = 0
cexdots= 0.8
pdf(width = 5, height = 9, file = "Figure_observed_predicted_loose_priors_independent_azswitch.pdf", onefile = T)

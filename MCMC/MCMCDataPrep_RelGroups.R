...
#to conduct subsample analysis uncomment and use this code
#pmsmall <- datahr %>% filter(Group == "pMSM")
#idsmsm <- sample(unique(pmsmall$Accession), floor(length(unique(pmsmall$Accession))/2), replace = F)
#datahr <-datahr%>% filter(Group == "Travel"|(Group == "pMSM" & Accession %in% idsmsm))
